<section>
    ...
    {!! Form::open(['route'=>['profile.password.update'], 'method'=>'put'], ['class'=>"mt-6 space-y-6"] ) !!}
    <div>
        <x-input-label for="current_password" :value="__('Настояща парола:')" />
        <x-text-input id="current_password" name="current_password" type="password" class="mt-1 block w-full" autocomplete="current-password" />
        <x-input-error :messages="$errors->get('current_password')" class="mt-2" />
    </div>

    <div>
        <x-input-label for="password" :value="__('Нова парола:')" />
